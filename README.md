...
Follow the instructions in class to run `slam_toolbox` to make a map. Save the map as `pure_pursuit.pgm` and `pure_pursuit.yaml`.

## III. Localization with Particle Filter

Follow the instructions in class to run `particle_filter` on the car using the new map you've made on pure_pursuit map.
...
- **Deliverable 1**: Commit your pure pursuit package to GitHub. Your commited code should run smoothly in simulation.
- **Deliverable 2**: Submit a link to a video on YouTube showing the car following waypoints in the simulator. 
